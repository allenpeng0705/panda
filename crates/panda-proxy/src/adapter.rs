...
fn parse_data_image_url(url: &str) -> Option<(String, String)> {
    ...
    let mime = meta.strip_suffix(";base64").or_else(|| meta.split(';').next())?;
        ...
        for call in tc {
            ...
            let (name, args_str) = if call.get("function").is_some() {
                ...
                (
                    ...
                    call.get("arguments").and_then(|x| x.as_str()).unwrap_or("{}"),
                )
            };
...
mod tests {
    ...
    fn maps_anthropic_tool_use_to_openai_tool_calls() {
        ...
        assert!(tc["function"]["arguments"].as_str().unwrap().contains("NYC"));
    }
}
